...
const ViewFollowing = (props) => {
  ...
  const getFollowing = async () => {
    const { data } = await API.get(
      `/get_following?id=${id}`, { headers: { "token": `Bearer ${token}` } }
    )
                ...
                <div class="card pro_card_top border-0 p-0 bg-white shadow-xss rounded-xxl">
                  ...
                  <div class="card-body d-block  pro_card shadow-none mb-0 p-0 border-top-xs">
                    <ul class="nav nav-tabs h55 d-flex product-info-tab border-bottom-0 ps-4 mt-0">
                      ...
                      <li class="active list-inline-item me-5"><a class="fw-700 font-xssss text-grey-500 pt-3 pb-3 ls-1 d-inline-block " href={`/ViewFollowers/${person?._id}`}>Followers</a></li>
                      <li class="list-inline-item me-5"><a class="fw-700 font-xssss text-grey-500 pt-3 pb-3 ls-1 d-inline-block active" href={`/ViewFollowing/${person?._id}`}>Following</a></li>
